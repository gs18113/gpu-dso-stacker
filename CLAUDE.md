...
- **DLT produces backward H directly**: row setup uses `(ref_x, ref_y)` as H input and `(src_x, src_y)` as output → null vector = backward map (ref → src). No post-inversion needed. `match_radius` is applied **before** RANSAC as a nearest-neighbour candidate gate; RANSAC random sampling operates only on the surviving candidate matches.
