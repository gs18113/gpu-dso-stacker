...
**배경 정규화** (`--bg-calibration`): 활성화 시 각 프레임의 배경 레벨과 신호 스케일을 워프 후 적분 전에 기준 프레임에 맞추어 정규화합니다. 중앙값과 MAD(중앙값 절대 편차)를 사용한 견고한 추정 방식입니다. 두 가지 모드: `per-channel`은 R/G/B를 독립적으로 정규화(유색 광공해 처리), `rgb`는 휘도 기반 통계를 모든 채널에 적용(색 비율 유지). 하늘 밝기가 프레임 간에 변하는 경우(구름, 달 뜨기, 고도 변화)에 유용합니다.
...
화이트 밸런스 (디베이어링 전 원시 베이어 모자이크에 적용):
      --white-balance <mode>     none | camera | auto | manual (기본값: none)
                                 camera: RAW 파일 메타데이터 사용
                                 auto: 그레이 월드 가정 (평균 R ~ G ~ B)
                                 manual: --wb-red/green/blue 곱셈기 사용
      --wb-red <float>           빨간색 채널 곱셈기 (기본값: 1.0)
      --wb-green <float>         녹색 채널 곱셈기 (기본값: 1.0)
      --wb-blue <float>          파란색 채널 곱셈기 (기본값: 1.0)

배경 정규화:
      --bg-calibration <mode>    none | per-channel | rgb (기본값: none)
                                 각 프레임의 배경을 적분 전 기준 프레임에 맞춰 정규화합니다.
